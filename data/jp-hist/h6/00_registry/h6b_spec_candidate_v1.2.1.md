# H6-B External Mechanism Validation — Spec Candidate v1.2.1（Specification-Closure PATCH，待外审）

版本：2026-09-07 v1.2.1 candidate（B0 v1.2 外审 REWORK 响应：3×P0/5×P1/3×P2 全闭）
状态：candidate——外审 PASS 后提升 FROZEN v1.2.1。
上游：①v1.2 candidate（16 节全量，本版基于其闭包修订；v1.2 文件只读保留）②《JP-HIST_H6B_B0_candidate_外部审核报告_20260907.md》（REWORK，P0×3/P1×5/P2×3，独立核实全属实）③执行评审书（R1-R8 仍有效）
元数据：spec_version=v1.2.1-candidate；candidate_date=2026-09-07；sha256 见 B0 v1.2.1 审核包 manifest。
...
骨架不变。变量全集见 h6b_variable_registry_v1.2.1.csv（37 行，19 列）。

## 3. 机制假说（FROZEN，同 v1.2 §3）+ 符号系统（v1.2.1 闭包 P1-2）
- H6B-H1..H5 不变。
- **符号系统机器化（新增 h6b_sign_registry_v1.2.1.csv）**：每个变量定义 (a) raw_series_definition、(b) deterioration_direction（恶化方向：↑ 或 ↓）、(c) sign_multiplier（raw→deterioration 变换乘子：恶化=正向时 +1，恶化=负向时 −1）、(d) expected_chain_sign（相邻链预期符号，全为 +1：恶化同向传导）。所有检验在 deterioration 变换后的序列上进行——**变量先乘 sign_multiplier 再进 robust-z/channel index**，Gate 判据统一为"θ 与预期同号且显著"。
...
## 16. Machine QC（v1.2.1 验收；对应外审 §十八 建议清单全项）
```
n_variables=37
n_gates=16
...
```
以上 11 项机器断言由 engine/QC 脚本输出（h6b_b0_v121_qc.txt），全 PASS 方可送审。
