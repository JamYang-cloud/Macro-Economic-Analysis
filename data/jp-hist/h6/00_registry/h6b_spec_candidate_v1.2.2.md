# H6-B External Mechanism Validation — Spec Candidate v1.2.2（SIGN/SEMANTIC CLOSURE PATCH，待外审）

版本：2026-09-07 v1.2.2 candidate（B0 v1.2.1 外审 REWORK 响应：新 P0×1/P1×2/P2×2 全闭——SIGN/SEMANTIC CLOSURE）
状态：candidate——外审 PASS 后提升 FROZEN v1.2.2。
上游：①v1.2.1 candidate（本版基于其 SIGN/SEMANTIC 闭包修订；v1.2.1 文件只读保留）②《JP-HIST_H6B_B0_v1.2.1_外部审核报告_20260907.md》（REWORK：sign orientation P0 + B3 semantic P1 + rehash P1 + P2×2，独立核实全属实）③执行评审书（R1-R8 仍有效）
元数据：spec_version=v1.2.2-candidate；candidate_date=2026-09-07；sha256 见 B0 v1.2.2 审核包 manifest。
...
- v1.2.2（本版）：v1.2.1 外审 REWORK——新 P0=sign registry 方向错误（deterioration_direction=↓ 但 sign_multiplier=+1，涉 Investment/Credit 主链端点，直接改变 B3/B4 符号与 Verdict）；P1×2（B3 demand proxy 必须冻结为 proxy-level 解释；修复后 closed set 同步重签 SHA）；P2×2（stance level/change 拆分报告；World GDP annual-step 标注低频限制）。全部关闭：sign registry/variable registry 37 行机器一致性修复 + 新 QC（sign_direction_multiplier_mismatch 等）+ B3 语义冻结 CREDIT_DEMAND_PROXY_EVIDENCE + closed set 重签。
...
骨架不变。变量全集见 h6b_variable_registry_v1.2.2.csv（37 行，19 列）。

## 3. 机制假说（FROZEN，同 v1.2 §3）+ 符号系统（v1.2.1 闭包 P1-2）
- H6B-H1..H5 不变。
- **符号系统机器化（v1.2.2 修复版 h6b_sign_registry_v1.2.2.csv）**：每个变量定义 (a) raw_series_definition、(b) deterioration_direction（恶化方向：↑ 或 ↓）、(c) sign_multiplier、(d) expected_chain_sign。
...
- Shock-adjusted 轨应用规则冻结（新 registry）：
  ...
  - **reporting limitation（外审 v1.2.1 P2）**：WB annual growth 年内平铺不产生真正季度全球冲击信息，Model A quarterly track 实为 annual-step control——最终报告必须标注 `GLOBAL_GROWTH_CONTROL_ANNUAL_STEP_QUARTERLY`（见 shock_adjustment_registry_v1.2.2.csv reporting_limitation 行）；
...
## 13. 数据源优先级（FROZEN；P2-1 闭包 → h6b_source_priority_registry_v1.2.1.csv）
...
## 16. Machine QC（v1.2.2 验收；外审 §十八 + v1.2.1 §15 建议全项）
```
n_variables=37
n_gates=16
sign_direction_multiplier_mismatch=0
sign_variable_registry_cross_mismatch=0
sign_expected_chain_invalid=0
gate_variable_unregistered=0
registry_duplicate_variable_id=0
...
```
以上 18 项机器断言由 engine/QC 脚本输出（h6b_b0_v122_qc.txt），全 PASS 方可送审。
